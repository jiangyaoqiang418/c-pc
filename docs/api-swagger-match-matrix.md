...
- 2026-08-29 前端稳定性收口：商品、订单、买手订单、售后、求购、理财详情/列表及订单群读取均补充最新请求保护与组件卸载取消，避免筛选或路由快速切换时旧响应覆盖新页面；商品详情将请求失败与商品不存在分开呈现，购物车按字符串化业务 ID 合并/更新/删除，通知未读数不会因重复回读降到负数；本轮追加钱包原始 `bizType/bizGroup` 透传与真实类型展示、订单/钱包多类型深分页按真实 `pageSize` 分批读取、登录失败 token 回滚、钱包首页部分并发失败隔离、商品/收藏/求购页码越界一次回退，充值链选择未选中状态改为 `undefined` 以消除 `Empty key`。Chrome 本地回归确认权限守卫、明确错误态和核心页面无新增控制台 warning/error。
