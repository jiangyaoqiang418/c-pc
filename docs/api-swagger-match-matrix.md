...
- CMS（公告、帮助、协议）和 AI 导购当前仅保留本地数据展示边界：可验证页面渲染、空态和错误处理，但不计入真实接口接入或真实数据回归；AI 页面已明确标识本地演示，后端提供 C 端契约后再替换单一 API 边界。本批读取页面已补取消旧请求、按钮无障碍语义和金额展示回归；未执行响应式验收，已执行限定的评价与测试环境资金写入回归。
