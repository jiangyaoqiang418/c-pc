...
- 2026-08-29 前端稳定性收口：商品、订单、买手订单、售后、求购、理财详情/列表及订单群读取均补充最新请求保护与组件卸载取消，避免筛选或路由快速切换时旧响应覆盖新页面；商品详情将请求失败与商品不存在分开呈现，购物车按字符串化业务 ID 合并/更新/删除，通知未读数不会因重复回读降到负数。Chrome 本地回归确认权限守卫、明确错误态和核心页面无新增控制台 warning/error。
