...
<template>
  <a-spin :loading="loading" style="width:100%">
    ...
    <a-form :model="form" layout="vertical">
      <a-row :gutter="12"><a-col :span="12"><a-form-item label="收货人" required><a-input v-model="form.receiverName" /></a-form-item></a-col><a-col :span="12"><a-form-item label="联系电话" required><a-input v-model="form.receiverPhone" :max-length="32" /></a-form-item></a-col></a-row>
      <a-form-item label="国家/地区" required><a-select :model-value="form.countryCode" allow-search @change="value => onCountryChange(String(value))"><a-option v-for="item in countries" :key="item.code" :value="item.code">{{ item.name }}{{ item.enName ? ` / ${item.enName}` : '' }}</a-option></a-select></a-form-item>
      <a-row v-if="hasRegion" :gutter="12"><a-col :span="8"><a-form-item label="省/州" required><a-select :model-value="form.provinceCode" @change="value => onProvinceChange(String(value))"><a-option v-for="item in provinces" :key="item.code" :value="item.code">{{ item.name }}</a-option></a-select></a-form-item></a-col><a-col v-if="needCity" :span="8"><a-form-item label="城市" required><a-select :model-value="form.cityCode" @change="value => onCityChange(String(value))"><a-option v-for="item in cities" :key="item.code" :value="item.code">{{ item.name }}</a-option></a-select></a-form-item></a-col><a-col v-if="needDistrict" :span="8"><a-form-item label="区/县" required><a-select v-model="form.districtCode"><a-option v-for="item in districts" :key="item.code" :value="item.code">{{ item.name }}</a-option></a-select></a-form-item></a-col></a-row>
      <a-row v-else :gutter="12"><a-col :span="12"><a-form-item label="省/州" required><a-input v-model="form.province" /></a-form-item></a-col><a-col :span="12"><a-form-item label="城市" required><a-input v-model="form.city" /></a-form-item></a-col></a-row>
      <a-form-item label="详细地址" required><a-textarea v-model="form.detailAddress" :rows="2" placeholder="街道、门牌号、楼栋等" /></a-form-item>
      <a-row :gutter="12"><a-col :span="12"><a-form-item label="邮编"><a-input v-model="form.postalCode" /></a-form-item></a-col><a-col :span="12"><a-form-item label="清关证件号"><a-input v-model="form.idCardNo" /></a-form-item></a-col></a-row>
