...
| 当前承接结论 | **部分完成**：评价和秒杀报名闭环均已收口；后台打款入口已恢复，提现已到“待打款”，等待完成真实 TRON 转账并提供对应交易哈希后在后台确认到账 |
...
| 真实验收 | `25/31` 项能力已完成完整真实验收，约 `81%`；主交易链路已完成双账号真实回归 |
| 本批完成 | 提现最低额按后端真实规则由 `10 U` 修正为 `20 U`；订单评价入口改为仅对 `/reviews/reviewable/page` 返回的顾客订单展示，避免买手视角和已评价订单触发无权或重复评价；评价删除、买手回复、申诉及后台裁定、秒杀报名/取消均已完成真实双端回归 |
| 本批验证 | `pnpm typecheck`、`pnpm test`（9 文件、24 项）、`pnpm check:swagger`、`git diff --check` 均通过；Chrome 真实测试账号已完成充值、提现驳回返还与审核通过回读、资金流水、评价写入/删除/回复/申诉及后台裁定、秒杀报名/取消，本批无新增 console warning/error |
| 浏览器边界 | 充值单 `2090357697803804673` 已回读测试环境模拟到账；提现单 `2090358267394482178` 已完成“申请 -> 后台驳回 -> 余额及资金流水返还回读”，提现单 `2090377523934879745` 已完成“申请 -> 后台审核通过 -> PC 待打款回读”。评价已验证顾客提交/删除、买手回复、申诉创建、后台裁定成立和买手端“已通过”回读；此前买手订单无权评价、已评价订单重复提交均由后端拒绝，前端入口现已按可评价列表收口 |
| 未完成与阻塞 | 提现尚未进行真实打款/链上到账：后台已恢复“确认打款”入口，但要求先完成真实 TRON 转账并填写对应交易哈希；禁止伪造哈希或未转账即标记到账。Notify WebSocket 未完成 `101 Switching Protocols`；CMS、AI 尚无 C 端契约；响应式验收按当前决定暂缓 |
| 后端需补充 | 真实 TRON 出款与 `txid`；Notify `/api/notify/im` WebSocket Upgrade；CMS（公告/帮助/协议）与 AI C 端契约；公开积分规则/VIP 全量配置；求购手动推送及推送日志字段；商品删除、买手押金划转、订单履约凭证与完整物流费用字段 |
| 下次继续 | 先使用待打款单 `2090377523934879745` 的真实 TRON `txid` 完成到账回读；再复验 Notify 实时到达；后端提供专题契约后依次接 CMS、AI 和缺失运营能力 |

演示账号没有真实 Token 时，真实接口返回登录失效码现在只展示失败态，不再清除演示会话或跳转登录页；真实账号仍按既有逻辑处理登录失效。评价卡片删除确认已补齐 Arco `Popconfirm` 全局注册，真实评价数据页面不再产生未知组件警告。

承接顺序：先执行 `git pull --ff-only origin main`，再运行 `pnpm install`（仅依赖缺失或锁文件变化时），读取本节后复用已有本地服务和 Chrome 登录态。当前可继续的真实闭环只剩待打款单的真实 TRON 交易/哈希和 Notify WebSocket 环境条件。

### 本批前端质量收口

- 商品列表、评价列表、资金流水、通知中心和买手售后列表的读取请求均只接受最新一次结果，并在新筛选、翻页或离开页面时取消旧请求，避免较慢响应覆盖最新结果或继续占用网络；加载、错误和重试仍保持原页面交互。
- Long 业务 ID 继续保持字符串原值；仅分页 `total` 这类统计值在 API adapter 统一转为 `number`，覆盖商品、订单、评价、售后、钱包、积分、通知、买手押金、求购、分类和理财分页，避免 Arco Pagination 收到字符串产生类型告警。真实登录态、钱包/流水、订单、商品、求购、买手押金、积分流水和 VIP 状态已独立使用 `Api.RealSession`、`Api.RealWallet`、`Api.RealOrder`、`Api.RealProduct`、`Api.RealPurchase`、`Api.RealBuyer`、`Api.RealPoint`、`Api.RealVip` 类型，不再借助 Mock `number` ID 模型伪装真实 Long。
