...
| 3 动态承运商 | orders/carriers、动态 string carrier、自定义名称/defaultCarrier/sortNo 已核对 | 已接入 | 动态选择与失败重读；详情历史名称快照 | 已登录真实返回 16 项；字典变更/发货未验 |
...
| 订单动作/物流 | order：`/orders/price`、`/orders/cancel`、`/orders/confirm`、`/orders/ship`、`/orders/logistics`、`/orders/logistics/track/create`、`/orders/logistics/exception/mark` | 状态/归属保护、动态carrier与名称快照、凭证/地址可选字段 | 卡片、详情、发货/物流弹窗调用 | 历史发货/轨迹/签收读写已验；新动态字典、非空采购/发货凭证、物流异常闭环待验 |
