...
| 本批完成 | 提现详情补齐 `fee/actualAmount/paidAt`；评价页接入筛选分页、删除、买手回复、申诉和申诉记录；删除零引用的旧售后 Mock adapter；开放已存在的积分明细、订单三方群和公告中心入口；补齐读取请求竞争保护、分页统计值适配、图片替代文本和未开放入口文案 |
| 本批验证 | `pnpm typecheck`、`pnpm test`（8 文件、20 项）、`pnpm check:swagger`、`git diff --check` 均通过；Chrome 真实测试账号已回归商品价格升序、评价读取、资金流水、通知仅未读、公告和 AI 页面，本批无新增 console warning/error、空白页或异常跳转 |
...
- 商品列表、评价列表、资金流水、通知中心和买手售后列表的读取请求均只接受最新一次结果，快速切换筛选或页码时不会被较慢的旧响应覆盖；加载、错误和重试仍保持原页面交互。
...
- 所有原生 `<img>` 已补充有意义的 `alt`；原先遗留的阶段占位入口改为“暂未开放”的禁用文案，不将没有接口或写入能力的功能误开放。
