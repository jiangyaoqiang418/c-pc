...
- 本批新增映射：`GET /user/withdraw/detail` 的 `fee`、`actualAmount`、`paidAt` 已完成类型和页面展示，真实测试账号已回读非空提现记录与详情；评价的删除、回复、申诉、申诉记录、状态/带图筛选和分页均已连接现有 order Swagger，已回读我发出、买手收到的非空数据及申诉空态。评价删除确认已补齐 `Popconfirm` 组件注册。演示账号无真实 Token 时，真实接口失败保留当前演示会话并展示错误态，不再误跳登录页。分页读取的 `total` 统一在 API adapter 转为数值，业务 Long ID 保持原值。
- 尚未完整验收：提现真实申请/打款/到账、评价删除/回复/申诉写入、秒杀报名写入、WebSocket 实时到达；CMS、AI 没有 C 端契约，继续保留 Mock，不伪造真实成功。
...
- CMS（公告、帮助、协议）和 AI 导购当前仅保留本地数据展示边界：可验证页面渲染、空态和错误处理，但不计入真实接口接入或真实数据回归；后端提供 C 端契约后再替换单一 API 边界。本批读取页面已补取消旧请求、按钮无障碍语义和金额展示回归；未执行响应式验收或任何业务写入。
