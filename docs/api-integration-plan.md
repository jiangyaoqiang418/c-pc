...
当前阻塞：Notify WebSocket 尚未完成 `101 Switching Protocols` 握手；秒杀缺启用场次和合格商品；充值/提现缺真实链上写入回归；CMS、AI 尚无 C 端 Swagger 契约。下一步在测试数据可回滚的前提下回归提现申请、评价删除/回复/申诉写入，再等待后台环境条件收口秒杀与实时 IM。
