...
| 真实验收 | `22/31` 项能力已完成完整真实验收，约 `71%`；主交易链路已完成双账号真实回归 |
| 本批完成 | 提现详情补齐 `fee/actualAmount/paidAt`；评价页接入筛选分页、删除、买手回复、申诉和申诉记录；删除零引用的旧售后 Mock adapter；开放已存在的积分明细、订单三方群和公告中心入口；补齐读取请求取消与竞争保护、分页统计值适配、图片替代文本、键盘焦点和未开放入口文案 |
| 本批验证 | `pnpm typecheck`、`pnpm test`（9 文件、24 项）、`pnpm check:swagger`、`git diff --check` 均通过；Chrome 真实测试账号已回归商品价格快速切换、评价读取、资金流水、通知仅未读、公告和 AI 页面，本批无新增 console warning/error、空白页或异常跳转 |
| 浏览器边界 | 已回读评价非空数据、申诉空态及提现非空记录；未执行提现申请、评价删除、回复或申诉等写入操作，不以页面只读回归替代写入验收 |
| 未完成与阻塞 | 提现、评价写入需可回滚测试数据和最终写入确认；秒杀缺启用场次与合格商品；Notify WebSocket 未完成 `101 Switching Protocols`；CMS、AI 尚无 C 端契约；响应式验收按当前决定暂缓 |
| 下次继续 | 条件满足后优先回归“提现申请 -> 后台处理 -> 余额/流水回读”和“评价写入 -> 列表/申诉记录回读”；再收口秒杀报名与 IM 实时到达 |
...
- CMS（公告、帮助、协议）与 AI 导购继续使用当前本地数据实现，仅作为页面展示和交互演示，不标记为真实接口完成，也不在真实接口失败时回退或伪造数据。本批未执行响应式验收，也未执行评价、资金、通知等业务写入。
