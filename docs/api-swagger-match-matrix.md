...
- 2026-08-29 数值展示边界收口：押金环图、买手工作台与押金管理统计、顾客订单统计、评分、理财曲线、锁仓进度、利息预估、年化、资产占比、通知/IM 未读数以及真实 adapter 数字时间戳展示统一对非法响应安全降级，避免 `NaN` 文本、无效 SVG 坐标、无效日期和错误百分比；求购大厅预算汇总遇到非法预算时降级为占位符，异常理财产品禁止提交，购物车价格/运费/税费异常时标记商品失效并阻止进入结算；合法数据的显示精度与业务操作保持不变。
