...
- Long 业务 ID 继续保持字符串原值；仅分页 `total` 这类统计值在 API adapter 统一转为 `number`，覆盖商品、订单、评价、售后、钱包、积分、通知、买手押金、求购、分类和理财分页，避免 Arco Pagination 收到字符串产生类型告警。真实登录态、钱包/流水和订单展示已独立使用 `Api.RealSession`、`Api.RealWallet`、`Api.RealOrder` 类型，不再借助 Mock `number` ID 模型伪装真实 Long。
