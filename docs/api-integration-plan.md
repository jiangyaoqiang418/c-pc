...
| 本批完成 | 提现详情补齐 `fee/actualAmount/paidAt`；评价页接入筛选分页、删除、买手回复、申诉和申诉记录；删除零引用的旧售后 Mock adapter；开放已存在的积分明细、订单三方群和公告中心入口 |
| 本批验证 | `pnpm typecheck`、`pnpm test`（5 文件、14 项）、`pnpm check:swagger`、`git diff --check` 均通过；Chrome 真实测试账号已回归评价三类读取与提现列表/详情，评价删除确认弹层可正常打开；另已巡检 39 个静态页面和 6 个动态详情场景，无新增 console warning/error、空白页或异常跳转 |
