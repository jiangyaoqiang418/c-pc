...
- 2026-08-26 WebSocket 复验：按后端整改文档以子协议 `['im', 'im.token.<token>']` 连接，已部署直连 `wss://testhou.merchantsale.store/api/notify/im` 与本地 `ws://localhost:5173/api/notify/im` 均协商回显 `im`、下推 `READY(heartbeatIntervalMs=25000)`，并完成 `PING/PONG`；后者仅用于 C-PC Vite 代理验证，并非本地后台地址。Chrome john 与 Safari mamba 的部署双会话已完成页面级实时到达、已读、撤回和断线补偿，测试消息已撤回。此前“未返回 101、网关阻塞”的记录仅为历史状态。
