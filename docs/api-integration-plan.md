...
| 口径 | 当前状态 |
|---|---|
| 承接分支 | `main`；当前 Long ID 类型隔离提交以 Git `HEAD` 为准，推送后以 `git pull --ff-only origin main` 同步 |
...
| 本批完成 | 真实登录态、钱包/流水、订单、商品、求购、买手押金、积分流水和 VIP 状态均已从 Mock `number` ID 模型隔离；评价删除、回复、申诉及后台裁定、秒杀报名/取消均已完成真实双端回归 |
| 本批验证 | `pnpm typecheck`、`pnpm test`（9 文件、24 项）、`pnpm check:swagger`、`git diff --check` 均通过；Chrome 已回归真实钱包、订单、商品与 IM REST。押金、积分与 VIP 最后一轮读取因 Chrome 会话中断，仅保留代码/自动检查证据待补 |
...
| 下次继续 | 先补押金、积分、VIP 的 Chrome 读取回归；再使用待打款单 `2090377523934879745` 的真实 TRON `txid` 完成到账回读，随后复验 Notify 实时到达 |
