...
- 2026-08-26 WebSocket 复验：按后端整改文档以子协议 `['im', 'im.token.<token>']` 连接，直连 `wss://testhou.merchantsale.store/api/notify/im` 与本地 `ws://localhost:5173/api/notify/im` 均协商回显 `im`、下推 `READY(heartbeatIntervalMs=25000)`，并完成 `PING/PONG`。此前“未返回 101、网关阻塞”的记录仅为历史状态；当前剩余的是浏览器双账号实时业务回归。
