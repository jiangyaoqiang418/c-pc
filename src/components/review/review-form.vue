...
function choosePhoto() {
  if (!props.submitting && !uploading.value && form.photoUrls.length < MAX_PHOTOS) fileInputRef.value?.click();
}
...
async function onPhotoSelected(event: Event) {
  ...
  input.value = '';
  if (!file || props.submitting || uploading.value) return;
...
function submit() {
  ...
  emit('submit', { ...form, photoUrls: [...form.photoUrls] });
}
</script>

<template>
  <a-card class="review-form" :body-style="{ padding: '24px 28px' }" :bordered="false" @pointerdown.capture="markInteracted" @keydown.capture="markInteracted" @focusin.capture="markInteracted">
    <div class="row">
      <span class="lbl">商品评分</span>
      <ReviewStars v-model:score="form.productScore" :mode="submitting ? 'readonly' : 'input'" size="lg" />
      <span class="score-label">{{ scoreLabel }}</span>
    </div>

    <div class="row">
      <span class="lbl">买手服务</span>
      <ReviewStars v-model:score="form.sellerScore" :mode="submitting ? 'readonly' : 'input'" size="lg" />
    </div>
    ...
    <div class="row col">
      ...
      <input ref="fileInputRef" class="file-input" type="file" accept="image/*" :disabled="submitting || uploading" @change="onPhotoSelected" />
      <div class="photos">
        <div v-for="(u, i) in form.photoUrls" :key="u" class="photo-item">
          <img :src="u" :alt="`评价图片 ${i + 1}`" />
          <button class="remove" type="button" :disabled="submitting" :aria-label="`删除评价图片 ${i + 1}`" :title="`删除评价图片 ${i + 1}`" @click="removePhoto(i)">✕</button>
        </div>
        <button
    ...
    <div class="actions">
      <a-button type="primary" size="large" :disabled="!canSubmit || uploading" :loading="submitting" @click="submit">
        提交评价
      </a-button>
    </div>
  </a-card>
