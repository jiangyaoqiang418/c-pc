...
| 当前承接结论 | **部分完成**：评价删除、回复、申诉和后台裁定已收口；提现已到“待打款”，等待后台开放测试环境打款执行入口，不得绕过页面直接调用资金接口 |
...
| 未完成与阻塞 | 提现尚未进行真实打款/链上到账：当前后台提现页面仅提供列表、详情和审核，未暴露 Swagger 的打款执行入口，不能绕过管理端界面直接调用资金接口；秒杀缺启用场次与合格商品；Notify WebSocket 未完成 `101 Switching Protocols`；CMS、AI 尚无 C 端契约；响应式验收按当前决定暂缓 |
| 下次继续 | 后台补齐或明确开放测试环境打款入口后，回归提现“待打款 -> 打款 -> 到账”；再收口秒杀报名与 IM 实时到达 |

演示账号没有真实 Token 时，真实接口返回登录失效码现在只展示失败态，不再清除演示会话或跳转登录页；真实账号仍按既有逻辑处理登录失效。评价卡片删除确认已补齐 Arco `Popconfirm` 全局注册，真实评价数据页面不再产生未知组件警告。

承接顺序：先执行 `git pull --ff-only origin main`，再运行 `pnpm install`（仅依赖缺失或锁文件变化时），读取本节后复用已有本地服务和 Chrome 登录态。当前可继续的真实闭环只剩后台打款入口、秒杀测试数据和 Notify WebSocket 环境条件。
