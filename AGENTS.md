...
- 用户已明确授权自主提交与推送：每批功能完成并通过适用验证后，可自行 commit 与 push；若构建失败，先修复或明确报告阻塞，不得推送失败构建。
...
- 用户已授权本项目自主推进：在“功能优先、不处理无关样式、真实接口不回退 Mock”的既定范围内，Codex 应自行完成代码修改、测试、构建、文档同步、提交，以及用户明确授权的推送，不逐项征求确认。
- 仅在必要前提缺失（后端接口、测试数据、权限或外部服务不可用）、下一步会实质扩大任务范围，或操作受平台安全规则限制时，才暂停并说明具体阻塞与所需条件。
- 浏览器中输入密码、验证码、支付信息、上传私密文件、授予设备权限及会产生外部业务写入的最终确认，仍遵循适用的浏览器安全要求；泛化自主授权不替代即时确认。
