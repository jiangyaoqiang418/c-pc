...
- 本批新增映射：`GET /user/withdraw/detail` 的 `fee`、`actualAmount`、`paidAt` 已完成类型和页面展示，真实测试账号已回读非空提现记录与详情；评价的删除、回复、申诉、申诉记录、状态/带图筛选和分页均已连接现有 order Swagger，已回读我发出、买手收到的非空数据及申诉空态。评价删除确认已补齐 `Popconfirm` 组件注册。演示账号无真实 Token 时，真实接口失败保留当前演示会话并展示错误态，不再误跳登录页。
