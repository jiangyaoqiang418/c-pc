  ...
  <div class="aftersale-create-page shop-container">
    <a-spin :loading="loading">
      <template v-if="order">
        ...
        <a-card class="order-card" :bordered="false"><div class="order-row"><img :src="order.productCover" class="cover" /><div><strong>{{ order.productTitle }}</strong><div class="meta"><OrderStatusTag :status="order.status" size="small" /> · 订单 {{ order.code }}</div></div><strong>U {{ order.totalAmount }}</strong></div></a-card>
