...
| 本批验证 | `pnpm typecheck`、`pnpm test`（5 文件、14 项）、`pnpm check:swagger`、`git diff --check` 均通过；Chrome 演示买手已验证评价三个页签、提现页失败态及控制台无错误 |
| 浏览器边界 | 当前 Chrome 没有真实测试登录态，未执行提现和评价新操作的非空数据、删除、回复或申诉写入回归；不以演示账号或 Mock 代替真实验收 |

演示账号没有真实 Token 时，真实接口返回登录失效码现在只展示失败态，不再清除演示会话或跳转登录页；真实账号仍按既有逻辑处理登录失效。

当前阻塞：Notify WebSocket 尚未完成 `101 Switching Protocols` 握手；秒杀缺启用场次和合格商品；充值/提现缺真实链上写入回归；CMS、AI 尚无 C 端 Swagger 契约。下一步先使用真实测试登录态回归提现详情和评价操作，再等待后台环境条件收口秒杀与实时 IM。
